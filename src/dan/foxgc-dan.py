...
def genmove(c, side_to_move):
  c.sendline('genmove ' + ('B' if side_to_move == 1 else 'W'))
  try: c.expect('\n= (.*)', timeout=-1)
  except:
    c.sendline('undo')
    c.expect('\n= (.*)', timeout=-1)
...
def play_game():
  ...
  while cheater_running:
    ...
    try:
      ...
      if best_move == 'PASS': messagebox.showinfo('Fox Go Cheater [2D]', 'AI has passed')
...
def start_playing(side):
  ...
  root.title('Fox Go Cheater [2D] (Match)')
...
def stop_playing():
  ...
  if cheater_thread and cheater_thread.is_alive():
    cheater_thread.join()
    root.title('Fox Go Cheater [2D] (Idle)')
